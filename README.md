...
SS(demand, L = 2, H = 0.4, service_level = 0.95)
```

Returns `$SS` (safety stock units) and `$annual_holding_cost` (= SS x H x N_periods).
...
ss <- SS(demand, L = 2, H = 0.4, alpha = 0.05)
ss$SS                # Safety stock in units
ss$annual_holding_cost

rop <- ROP(demand, L = 2, H = 0.4, alpha = 0.05)
