...
test_that("WW rejects non-numeric demand", {
  expect_error(WW("abc", 54, 0.4), "demand must be a numeric vector")
})

test_that("WW rejects demand with NA", {
  expect_error(WW(c(1, NA, 3), 54, 0.4), "demand must not contain NA")
})

test_that("WW rejects negative demand", {
  expect_error(WW(c(1, -2, 3), 54, 0.4), "demand must not contain negative")
})

test_that("WW rejects non-positive S", {
  expect_error(WW(c(10, 20), 0, 0.4), "S must be a single positive number")
  expect_error(WW(c(10, 20), -1, 0.4), "S must be a single positive number")
})

test_that("WW rejects non-positive H", {
  expect_error(WW(c(10, 20), 54, 0), "H must be a single positive number")
  expect_error(WW(c(10, 20), 54, -1), "H must be a single positive number")
})

test_that("WW rejects non-numeric S and H", {
  expect_error(WW(c(10, 20), "a", 0.4), "S must be a single positive number")
  expect_error(WW(c(10, 20), 54, "b"), "H must be a single positive number")
})
...
test_that("plot.WW does not error", {
  result <- WW(c(10, 62, 12, 130), S = 54, H = 0.4)
